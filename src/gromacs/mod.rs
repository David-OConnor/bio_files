...
//! 2. Call [`GromacsInput::save`] to write inputs to a directory, or
...
impl GromacsInput {
    ...
    ///
    /// Requires `gmx` (GROMACS) to be available on the system `PATH`.
    ///
    /// Steps:
...
fn run_gmx(dir: &Path, args: &[&str]) -> io::Result<()> {
...
fn run_gmx_stdin(dir: &Path, args: &[&str], stdin_data: &[u8]) -> io::Result<()> {
