...
/// One dense 3-D brick of map values.
#[derive(Debug)]
...
impl DensityCube {
    ...
    pub fn new(
        ...
        for atom_posit in atom_posits {
            ...
            min_f = Vec3::new(min_f.x.min(f.x), min_f.y.min(f.y), min_f.z.min(f.z));
            max_f = Vec3::new(max_f.x.max(f.x), max_f.y.max(f.y), max_f.z.max(f.z));
        }
        ...
        min_f = Vec3::new(
            ...
            (min_f.y - margin_f.y).max(0.0),
            (min_f.z - margin_f.z).max(0.0),
        );

        // upper bound: add margin, then clamp each component to ≤ 1.0
        max_f = Vec3::new(
            (max_f.x + margin_f.x).min(1.0),
            (max_f.y + margin_f.y).min(1.0),
            (max_f.z + margin_f.z).min(1.0),
        );

        // ——————————————————————————————————————————
        // 2.  Convert those bounds to voxel indices
        ...
        let mut hi_i = [
            to_idx(max_f.x, hdr.mx),
            to_idx(max_f.y, hdr.my),
            to_idx(max_f.z, hdr.mz),
        ];

        // Make inclusive range, then dimensions
        let dims = [
        ...
        // ——————————————————————————————————————————
        // 3.  Cartesian coordinate of voxel (0,0,0)
        //     We need the *centre* (add 0.5 index)
